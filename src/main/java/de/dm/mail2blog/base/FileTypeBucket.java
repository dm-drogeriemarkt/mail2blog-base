...
public class FileTypeBucket {
    ...
    @SneakyThrows(FileTypeBucketException.class)
    public static FileTypeBucket defaultBucket() {
        String allowedFileTypes = new Scanner(
            FileTypeBucket.class.getClassLoader().getResourceAsStream("filetypes.txt"),
            "UTF-8"
        ).useDelimiter("\\A").next();

        return FileTypeBucket.fromString(allowedFileTypes);
    }
